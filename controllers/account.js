...
module.exports = function(app) {
  ...
  app.post('/account/login', function(req, res) {
    ...
    app.libs.authenticate(user, req.body.password, function(err, user) {
      if (user) {
        app.libs.addSessionUser(req, user, rememberMe, function(sessionUser) {
  ...
  app.post('/account/edit', app.libs.restrict, function(req, res) {
    ...
    if (oldPassword.length > 0) {
      ...
      app.factory.users.addAltsToUserId(req.session.user.id, alts, function(
        user
      ) {
      ...
      app.factory.users.addAltsToUserId(req.session.user.id, [], function(
        user
      ) {
        app.libs.addSessionUser(req, user, false, function() {
    ...
    app.libs.authenticate(user, oldPassword, function(err, user) {
      if (!user) {
        res.render('account/edit', {
          err: 'Your old password was incorrect',
          msg: ''
        })
      } else {
