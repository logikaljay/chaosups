...
module.exports = function(app) {
  ...
  app.get('/items/unsent', app.libs.restrictAdmin, function(req, res) {
    app.factory.items.getByState(2, function(items) {
      items = items.sort((a, b) => {
        if (a.currentBid.endDate < b.currentBid.endDate) {
          return -1
        } else if (b.currentBid.endDate < a.currentBid.endDate) {
          return 1
        }
  ...
  app.get('/items/similar/:name', app.libs.restrict, function(req, res) {
    var name = req.params.name

    app.factory.items.getByName(name, items => {
      res.json(items)
    })
  })
}
